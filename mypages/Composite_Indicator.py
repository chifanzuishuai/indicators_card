...
def use_composite_indicator():

    df_database = pd.read_csv("./指标字典.csv")

    selected_department_name = df_database[df_database['指标类型'] == '复合指标']['归属部门'].unique()
    ...
    selected_index_name = df_database[(df_database['指标类型'] == '复合指标') &  (df_database['归属部门'] == selected_department)]['指标名称']
    ...
    # 筛选后指标信息
    index_value = df_database[(df_database['指标类型'] == '复合指标') & (df_database['指标名称'] == selected_name) & (df_database['归属部门'] == selected_department)]
